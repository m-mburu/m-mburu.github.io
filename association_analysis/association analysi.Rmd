...
dat <- dat[Item != "NONE"]
head(dat[sample(1:nrow(dat), 10)]) %>% kable()
...
head(dat2[, c(1:3, sample_cols), with = F]) %>% kable()
```
...
data.frame(item = names(n_transacts_item_in),
    ...
    arrange(desc(number)) %>% head(10) %>% kable()
...
head(dat2[sample(1:nrow(dat2), 10), .(Transaction, number_items,items_bought)]) %>%kable

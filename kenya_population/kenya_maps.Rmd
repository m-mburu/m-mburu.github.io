...
map4 <- tm_shape(pop_crops_counties_not_mbp)+
  ...
  tm_fill(col = "crops")+
  #tm_bubbles(size = "Perc", col = "gold")+
  tm_layout(title = "Popular crop grown per county Kenya apart from maize, beans or potatoes",
            title.position = c(0.2, "top"))
...
kenya_sp <- as(kenya_counties, "Spatial" )
setDT(kenya_counties)
...
```{r}
#extracting values
kenya_counties[, wasting_prev := raster::extract(world_wasting,
                                                 kenya_sp, weights=FALSE, fun=median,
                                                 na.rm = T)]

kenya_counties[, underweight_prev := raster::extract(world_underwight,
                                                     kenya_sp, weights=FALSE, fun=median,
                                                     na.rm = T)]

kenya_counties[, stunt_prev := raster::extract(world_stunting,
                                               kenya_sp, weights=FALSE, fun=median,
                                               na.rm = T)]
...
```{r}
kenya_counties[, OBJECTID := county]
kenya_counties_st <- st_set_geometry(kenya_counties, "geometry")
```


```{r}
map4 <- tm_shape(kenya_counties_st)+
  tm_borders(col = "gold")+
  tm_fill(col = "stunt_prev", id = "county", 
          title = "DF")

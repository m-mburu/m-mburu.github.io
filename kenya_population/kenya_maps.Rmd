...
```{r}
stunt_world <- raster::raster("stunt_post_means.TIF")
```
...
proj4string = as.character(projection(stunt_world))
kenya_shapefile <- st_set_geometry(kenya_shapefile, "geometry")
kenya_shapefile <- st_set_crs(kenya_shapefile, value = proj4string)
kenya_sp <- as(kenya_shapefile, "Spatial") 
```


```{r}
setDT(kenya_shapefile)
kenya_shapefile[, stunt_prev := raster::extract(stunt_world, kenya_sp,
                                               weights=FALSE, fun=mean,
                                               na.rm= T)]
kenya_shapefile[, value_var := paste(county, stunt_prev)]
kenya_shapefile <- st_set_geometry(kenya_shapefile, "geometry")
...
```{r}
ttm()
tm_shape(kenya_shapefile)+
  tm_polygons(col = "stunt_prev", id = "value_var" )
```

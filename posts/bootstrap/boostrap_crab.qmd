...
1)  what bootstrap is,
2)  parametric vs nonparametric bootstrap,
3)  how to construct bootstrap estimates and confidence intervals,
4)  bootstrap hypothesis tests (one-sample $t$ as the simplest example),
5)  an advanced likelihood-ratio bootstrap test on crab data,
...
-   Often excellent for generating **null distributions**.
...
list(
  mean = t_obs_mean,
  mean_boot_se = se_mean_np,
  mean_boot_bias = bias_mean_np,
  mean_boot_ci_95 = ci_mean_np,
  median = t_obs_median,
  median_boot_ci_95 = ci_median_np
)
```
...
- The 95% percentile bootstrap CI for the mean is (`r round(ci_mean_np[1], 2)`, `r round(ci_mean_np[2], 2)`). Interpreted literally: if we repeated the whole sampling process many times, about 95% of such intervals would contain the true population mean.
- The 95% percentile bootstrap CI for the median is (`r round(ci_median_np[1], 2)`, `r round(ci_median_np[2], 2)`). Comparing the widths of the mean vs median CIs gives you a feel for which statistic is more variable for this sample size.
...
se_mean_par <- sd(boot_mean_par)
ci_mean_par <- percentile_ci(boot_mean_par)

list(
  mean_boot_se_parametric = se_mean_par,
  mean_boot_ci_95_parametric = ci_mean_par
)
```
...
- The parametric bootstrap 95% CI for the mean is (`r round(ci_mean_par[1], 2)`, `r round(ci_mean_par[2], 2)`).

...
-   The observed test statistic is \$t\_{obs} = \$ `r round(t_obs, 3)`. The bootstrap p-values estimate $\Pr(|T^*| \ge |t_{obs}| \mid H_0)$ by simulation.
...
-   The LRT statistic is $T = 2(\ell_{full} - \ell_{red})$, reported here as `$\\chi^2$`. For these data, \$T\_{obs} = \$ `r round(T_obs, 3)`.
